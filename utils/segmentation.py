...
def detectArucoSetSquare(im, outerLength=15., markerIds=[1, 0, 2], dictionary=cv2.aruco.DICT_4X4_50):
    ...
    if(len(markerIds) is not 3):
        raise ValueError('markersIds must contain 3 ids', markersIds)
    ...
    #print(ids)
    nb_ids = len([x for x in ids if x in markerIds])
    if(nb_ids != len(markerIds)):
        print("Error : {0} marker{1} out of {2} {3} found !".format(nb_ids,
                                                                    "" if nb_ids == 1 else "s",
                                                                    len(markerIds),
                                                                    "was" if nb_ids == 1 else "were"))

        for t in range(180,230,10):
...
def extractShape(im, setSquare=True, outerLength=15., markers=[1,0,2], arucoDict=aruco.DICT_4X4_50, object=None, ellipseSize=11, blurSize=60):
